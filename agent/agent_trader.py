    ...
    def _format_account_info(self, account_info, positions):
        ...
        if positions:
            ...
            for coin, pos in positions.items():
                # 计算仓位详细信息
                position_value = pos.get('position_value', abs(pos['size']) * pos['current_price'])
                margin_used = pos.get('margin_used', position_value / pos['leverage'] if pos['leverage'] > 0 else 0)
                pnl_percentage = (pos['unrealized_pnl'] / (abs(pos['size']) * pos['entry_price'])) * 100 if pos['size'] != 0 and pos['entry_price'] != 0 else 0
                ...
                prompt += f"  Mark Price: {pos.get('mark_price', pos['current_price']):.4f}\n"
...
IMPORTANT: Only trades with confidence >= 75% will be executed. If your confidence is below 75%, the trade will be ignored for safety reasons. Be honest about your confidence level."""
                ...
                if in_decisions:
                        ...
                        try:
                            ...
                            if confidence_match:
                                ...
                                if current_coin in decisions:
                                    ...
                                    self.logger.debug(f"解析到 {current_coin} 信心度: {confidence:.2%}")
                    ...
                    if current_coin and 'quantity:' in line.lower():
                        try:
                            ...
                            quantity_match = re.search(r'quantity:\s*(\d+(?:\.\d+)?)', line, re.IGNORECASE)
                            if quantity_match:
                                ...
                                if quantity > 0:  # 设置合理的数量范围
                                    if current_coin in decisions:
                                        ...
                                        self.logger.debug(f"解析到 {current_coin} 数量: {quantity}")
                                else:
    ...
    def execute_trading_decisions(self, decisions, gate=0.75):
        """执行交易决策"""
        executed_trades = []
        ...
        try:
            ...
            self.logger.info(f"账户状态正常: 可用余额 {account_info['free_usdt']:.2f} USDT")
        except Exception as e:
        ...
        if current_positions and len(current_positions) > 0:
            self.logger.warning(f"当前已有 {len(current_positions)} 个持仓，需要先平仓才能开新仓")
            # 平仓所有现有持仓
            try:
                closed_positions = self.okx.close_all_positions()
                for close_result in closed_positions:
                    if close_result['success']:
                        self.logger.info(f"成功平仓 {close_result['coin']} - 订单ID: {close_result['order_id']}")
                        # 记录平仓交易
                        trade_record = {
            ...
            try:
                ...
                if signal == 'BUY':
                    ...
                    if quantity > 0 and confidence >= gate:
                        ...
                        current_positions = self.okx.get_positions()
                        if current_positions and len(current_positions) > 0:
                            self.logger.warning(f"尝试买入 {coin} 但已有持仓，跳过此交易")
                            continue
                        try:
                            leverage = 10  # 固定10倍杠杆
                            ...
                            if order and 'id' in order:
                                ...
                                executed_trades.append(trade_record)
                                self.state_manager.add_trade_record(trade_record)
                                self.logger.info(f"买入 {coin} {quantity} (信心度: {confidence:.1%}) - 订单ID: {order['id']}")
                            else:
                        ...
                        self.logger.info(f"跳过买入 {coin}: 数量={quantity}, 信心度={confidence:.1%} (需要>={gate:.1%})")
                    ...
                    if quantity > 0 and confidence >= gate:
                        ...
                        current_positions = self.okx.get_positions()
                        if current_positions and len(current_positions) > 0:
                            self.logger.warning(f"尝试卖出 {coin} 但已有持仓，跳过此交易")
                            continue
                        try:
                            leverage = 10  # 固定10倍杠杆
                            order = self.okx.place_order(
                            ...
                            if order and 'id' in order:
                                ...
                                executed_trades.append(trade_record)
                                self.state_manager.add_trade_record(trade_record)
                                self.logger.info(f"卖出 {coin} {quantity} (信心度: {confidence:.1%}) - 订单ID: {order['id']}")
                            else:
                        ...
                        self.logger.info(f"跳过卖出 {coin}: 数量={quantity}, 信心度={confidence:.1%} (需要>={gate:.1%})")
