...
def format_price_change(current_price, open_price):
    ...
    # 根据涨跌选择颜色
    if change > 0:
        color = Colors.RED  # 红色表示上涨
        sign = '+'
    elif change < 0:
        color = Colors.GREEN  # 绿色表示下跌
        sign = ''
    else:
        color = Colors.WHITE
        sign = ''
    
    return f"${current_price:>7.2f} {color}{sign}{change_pct:>6.2f}%{Colors.RESET}"
...
def format_volume_ratio(estimated_volume, avg_volume):
    ...
    return f"{color}{ratio:>6.1f}%{Colors.RESET}"
...
def format_rsi_trend(rsi_prev, rsi_current):
    ...
    return f"{rsi_prev:>5.1f} {color}{arrow}{Colors.RESET} {rsi_current:>5.1f}"


def format_macd_info(dif, dea, dif_slope):
    """
    格式化MACD信息
    ...
    if dif_slope is not None:
        if dif_slope > 0:
            ...
            slope_sign = ''
        slope_str = f"{slope_color}{slope_sign}{dif_slope:.2f}{Colors.RESET}"
    else:
        slope_str = "N/A"
...
def print_header():
    """打印表头"""
    print(f"\n{'='*120}")
    print(f"{'股票':<6} | {'价格':>8} {'涨幅':>8} | {'量比':>12} | {'RSI8(前->今)':^20} | {'MACD指标':<40} | {'信号'}")
    print(f"{'='*120}")
