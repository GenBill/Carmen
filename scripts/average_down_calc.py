#!/usr/bin/env python3
"""加仓点位与股数计算：5 笔等额资金，模式 A / 模式 B。"""
from __future__ import annotations

import argparse
import sys
from dataclasses import dataclass


TRANCHES = 5
TRANCHES_A = 6


@dataclass(frozen=True)
class Tranche:
    index: int
    step_drop_pct: float
    price: float
    amount: float
    shares: int
    spent: float


def _build_row(
    index: int,
    step_drop_pct: float,
    price: float,
    per_amount: float,
) -> Tranche:
    shares = int(per_amount // price)
    return Tranche(
        index=index,
        step_drop_pct=step_drop_pct,
        price=price,
        amount=per_amount,
        shares=shares,
        spent=shares * price,
    )


def calc_mode_a(current_price: float, total_funds: float) -> list[Tranche]:
    """第 1 笔现价买入，之后每笔相对上一笔再跌 16%。"""
    per_amount = total_funds / TRANCHES_A
    rows: list[Tranche] = []
    price = current_price
    rows.append(_build_row(1, 0.0, price, per_amount))
    for i in range(2, TRANCHES_A + 1):
        price = price * 0.84
        rows.append(_build_row(i, 16.0, price, per_amount))
    return rows


def calc_mode_b(current_price: float, total_funds: float) -> list[Tranche]:
    """每笔相对上一笔（首笔相对现价）再跌 16%。"""
    per_amount = total_funds / TRANCHES
    rows: list[Tranche] = []
    price = current_price
    for i in range(1, TRANCHES + 1):
        price = price * 0.84
        rows.append(_build_row(i, 16.0, price, per_amount))
    return rows


COL_SEP = "  |  "


def _styled(text: str, *, bold: bool = False, dim: bool = False) -> str:
    if not sys.stdout.isatty():
        return text
    if bold:
        return f"\033[1m{text}\033[0m"
    if dim:
        return f"\033[2m{text}\033[0m"
    return text


def _cell(value, fmt: str, *, bold: bool = False, dim: bool = False) -> str:
    return _styled(format(value, fmt), bold=bold, dim=dim)


def print_mode(title: str, rows: list[Tranche], total_funds: float) -> None:
    total_spent = sum(r.spent for r in rows)
    total_shares = sum(r.shares for r in rows)
    unused = total_funds - total_spent

    print(title)
    print("-" * 68)
    header = (
        f"{_styled(f'{'笔次':>4}', dim=True)}  "
        f"{_styled(f'{'跌幅':>3}', dim=True)}"
        f"{COL_SEP}"
        f"{_styled(f'{'加仓价':>7}', bold=True)}  "
        f"{_styled(f'{'股数':>5}', bold=True)}"
        f"{COL_SEP}"
        f"{_styled(f'{'实际花费':>6}', dim=True)}  "
        f"{_styled(f'{'平均成本':>8}', dim=True)}"
    )
    print(header)
    print("-" * 68)
    cum_spent = 0.0
    cum_shares = 0
    for r in rows:
        cum_spent += r.spent
        cum_shares += r.shares
        avg_cost = cum_spent / cum_shares if cum_shares > 0 else 0.0
        row = (
            f"{_styled(f'{r.index:>4}', dim=True)}  "
            f"{_styled(f'{r.step_drop_pct:>6.0f}%', dim=True)}"
            f"{COL_SEP}"
            f"{_styled(f'{r.price:>10.2f}', bold=True)}  "
            f"{_styled(f'{r.shares:>7}', bold=True)}"
            f"{COL_SEP}"
            f"{_styled(f'{r.spent:>10.2f}', dim=True)}  "
            f"{_styled(f'{avg_cost:>12.2f}', dim=True)}"
        )
        print(row)
    print("-" * 68)
    print(
        f"合计: 股数 {total_shares}  |  实际花费 {total_spent:.2f}  |  "
        f"未动用资金 {unused:.2f}"
    )
    print()


def run(current_price: float, total_funds: float) -> None:
    if current_price <= 0:
        raise ValueError("当前价格必须大于 0")
    if total_funds <= 0:
        raise ValueError("加仓资金必须大于 0")

    per_amount = total_funds / TRANCHES
    print()
    print(f"当前价格: {current_price:.2f}")
    print(f"加仓总资金: {total_funds:.2f}  (每笔分配 {per_amount:.2f})")
    print()

    mode_a = calc_mode_a(current_price, total_funds)
    mode_b = calc_mode_b(current_price, total_funds)

    print_mode(
        "模式 A：分 6 笔等额加仓，第 1 笔现价买入，之后每笔相对上一笔再跌 16%",
        mode_a,
        total_funds,
    )
    print_mode(
        "模式 B：每笔相对上一笔再跌 16%",
        mode_b,
        total_funds,
    )


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="计算 5 笔等额加仓的点位与股数（模式 A / 模式 B）",
    )
    parser.add_argument("-p", "--price", type=float, help="当前标的价格")
    parser.add_argument("-f", "--funds", type=float, help="用于加仓的总资金")
    return parser.parse_args()


def prompt_float(label: str) -> float:
    while True:
        raw = input(f"{label}: ").strip()
        try:
            value = float(raw)
        except ValueError:
            print("请输入有效数字")
            continue
        if value <= 0:
            print("必须大于 0")
            continue
        return value


def main() -> None:
    args = parse_args()
    if args.price is not None and args.funds is not None:
        run(args.price, args.funds)
        return

    print("加仓计算（资金均分 5 笔）")
    price = args.price if args.price is not None else prompt_float("当前标的价格")
    funds = args.funds if args.funds is not None else prompt_float("用于加仓的资金")
    run(price, funds)


if __name__ == "__main__":
    main()
